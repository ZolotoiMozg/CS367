...
//Other files:      LinkedLoop.java, LinkedLoopIterator.java, EmptyLoopException.java
//Semester:         CS367 Summer 2014
//
//Author:           Stefen Showers (Denzil Showers)
//Email:            stefen.s@gmail.com
//CS Login:         showers3
...
/**
 * The MessageLoopEditor class creates an empty Loop of messages and then 
 * prompts the user to enter commands in order to edit and traverse the messages, 
 * which it processes until the user types q for quit.
 *
        ...
        while (again) {
            ...
            if (input.length() > 0) {
                ...
                case '?':
                    ...
                    break;
                    /**
                     * If the message loop is empty, add the new message to the 
                     * loop and make it the current message. Otherwise, add the 
                     * new message immediately after the current message and 
                     * make the new message the new current message. In either 
                     * case, display the current context (see note below).
                     */
                case 'a':
                    if (remainder.length() > 0) {
                        //if 0 or 1 message already in messages, just insert
                    	if (messages.size() < 2){
                        	messages.insert(remainder);
                        } else {
                        	//In order to insert after the current, remove 
                        	//current and store to temp, then insert the message,
                        	//re-insert temp, and finally move forward so the
                        	//message is new current
                        	String temp = messages.removeCurrent();
                        	messages.insert(remainder);
                        	messages.insert(temp);
                        	messages.forward();
                        }
                    	/**  DISPLAYING CURRENT IN CONTEXT
                    	 * Move backward and print, then move forward and print 
                    	 * surrounded by ===[ ]===, then move forward and print, 
                    	 * then move backward (back to the original current).
                    	 */
                    	//Only print message before current if 3 or more exist
                    	if (messages.size() > 2){
                    		messages.backward();
                    	...
                    	System.out.println("===[ " + messages.getCurrent() + " ]===");
                    	//Only print message after current if 2 or more exist
                    	if (messages.size() > 1){
                    		messages.forward();
                        	System.out.println("     " + messages.getCurrent());
                        	messages.backward();
                     ...
                     * If the message loop is empty, add the new message to the 
                     * loop and make it the current message. Otherwise, add the 
                     * new message immediately before the current message and 
                     * make the new message the new current message. In either 
                     * case, display the current context (see note below).
                     */
                case 'b':
                ...
                case 'd': 
                     ...
                     * first message that contains the given string.  If no 
                     * message containing string is found, display "not found"; 
                     * otherwise, make that message the current message and 
                ...
                case 'f':
                    if (remainder.length() > 0) {
                    	if (messages.isEmpty()){
                    		...
                    		boolean done = false;
                    		int i = 0;
                    		while(i < messages.size() && !done){
                    			//start with message after current
                    			messages.forward();
                    			if (messages.getCurrent().contains(remainder) && !done){
                    				/**  DISPLAYING CURRENT IN CONTEXT
                                	 * Move backward and print, then move 
                                	 * forward and print surrounded by ===[ ]===, 
                                	 * then move forward and print, then move 
                                	 * backward (back to the original current).
                                	 */
                                	//Only print message before current if 3 or more exist
                                	if (messages.size() > 2){
                                		messages.backward();
                    			...
                    			i++;
                    			if (!done) messages.forward();
                    		}
                    		if (!done) System.out.println("not found");
                    	}
                    } else {
                        System.out.println("invalid command");
                    }
                    ...
                    /**
                     * If the message loop is empty, display "no messages". 
                     * Otherwise, remove the current message from the message 
                     * loop. If the message loop becomes empty as a result of 
                     * the removal, display "no messages". Otherwise, make the 
                     * message after the removed message the current message and 
                     * display the current context (see note below).
                     */
                case 'r':
                    if (messages.isEmpty()){
                    	...
                    	if (messages.isEmpty()){
                        	...
                        	/**  DISPLAYING CURRENT IN CONTEXT
                        	 * Move backward and print, then move forward and 
                        	 * print surrounded by ===[ ]===, then move forward 
                        	 * and print, then move backward (back to the 
                        	 * original current).
                        	 */
                        	//Only print message before current if 3 or more exist
                        	if (messages.size() > 2){
                    ...
                    /**
                     * If the message loop is empty, display "no messages". 
                     * Otherwise, update the current message by replacing it 
                     * with the new message and display the current context 
                     * (see note below).
                     */
                case 'u':
                    if (remainder.length() > 0) {
                ...
                case '>':
                    ...
                    /**
                     * If the message loop is empty, display "no messages". 
                     * Otherwise, go backward to the previous message in the 
                     * loop and display the current context (see note below).
                     */
                case '<':
                	if (messages.isEmpty()){
                    	System.out.println("no messages");
